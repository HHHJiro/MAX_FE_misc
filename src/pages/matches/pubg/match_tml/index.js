...
var scoreRankDataUrl = PRODUCTION ? '//dotamax.net/static/pubg_league_5.json?league_id=5' : 'https://heybox.tt.maxjia.com/game/pubg_league/data/?league_id=2'
if (typeof document === 'object') {
  window.onload = function () {
    var time = new Date().getTime()
    // $.get( pubgDataUrl + '?' + time, function (calanderData) {
      $.get(scoreRankDataUrl + '&time=' + time, function (data) {
        var res = data.result
        // res.calander = calanderData.calander
        var app = new App(res)
      })
    // })
  }
